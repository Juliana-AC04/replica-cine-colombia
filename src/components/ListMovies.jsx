  ...
  return (
    <>
      {movies.slice(0, 4).map((movie) => (
        <li
          key={movie.id}
          className={`flex flex-row gap-4 p-4 mb-4 rounded-md ${
            'lg:w-72 lg:flex-col lg:bg-transparent lg:shadow-none'
        ...
        >
          ...
          <figcaption className="flex-col">
            ...
            <div className="flex flex-wrap text-[0.6875rem] mt-3 gap-2">
              <p
                className={`bg-zinc-200 px-3 shadow-md rounded-md p-1 ${
                  movie.adult
                    ? "text-blue-500"
...
export default function ListMovies() {
  ...
  useEffect(() => {
    const showData = async () => {
      try {
        ...
        // Obtener y almacenar la duración de cada película
        const runtimeData = {};
  ...
  return (
    <div className={`flex flex-col gap-2 font-inter ${'lg:flex'}`}>
      ...
      <ul className={`w-10/12 gap-2 mx-auto ${" lg:w-11/12 lg:flex lg:justify-around lg:pb-8"}`}>
